...
void lite::utils::draw_axis_inplace(cv::Mat& mat_inplace, const types::EulerAngles& euler_angles,
                                    float size, int thickness)
{
  ...
  const int y1 = static_cast<int>(size * (std::cos(pitch) * std::sin(roll) +
                                          std::cos(roll) * std::sin(pitch) * std::sin(yaw))) +
                 tdy;
  // Y-Axis | drawn in green
  const int x2 = static_cast<int>(-size * std::cos(yaw) * std::sin(roll)) + tdx;
  const int y2 = static_cast<int>(size * (std::cos(pitch) * std::cos(roll) -
  ...
  cv::line(
    mat_inplace, cv::Point2i(tdx, tdy), cv::Point2i(x1, y1), cv::Scalar(0, 0, 255), thickness);
  cv::line(
    mat_inplace, cv::Point2i(tdx, tdy), cv::Point2i(x2, y2), cv::Scalar(0, 255, 0), thickness);
  cv::line(
    mat_inplace, cv::Point2i(tdx, tdy), cv::Point2i(x3, y3), cv::Scalar(255, 0, 0), thickness);
}

cv::Mat lite::utils::draw_axis(const cv::Mat& mat, const types::EulerAngles& euler_angles,
                               float size, int thickness)
{
...
cv::Mat lite::utils::draw_landmarks(const cv::Mat& mat, types::Landmarks& landmarks)
{
  ...
  cv::Mat mat_copy = mat.clone();
  for (const auto& p : landmarks.points) cv::circle(mat_copy, p, 2, cv::Scalar(0, 255, 0), -1);
  return mat_copy;
}
...
void lite::utils::draw_landmarks_inplace(cv::Mat& mat, types::Landmarks& landmarks)
{
  ...
  for (const auto& p : landmarks.points) cv::circle(mat, p, 2, cv::Scalar(0, 255, 0), -1);
}
...
cv::Mat lite::utils::draw_boxes(const cv::Mat& mat, const std::vector<types::Boxf>& boxes)
{
  cv::Mat save;
  mat.copyTo(save);
  if (boxes.empty()) return save;

  for (const auto& box : boxes) {
    if (box.flag) {
      cv::rectangle(save, box.rect(), cv::Scalar(255, 255, 0), 2);
      if (box.label_text) {
        std::string label_text(box.label_text);
        label_text = label_text + ":" + std::to_string(box.score).substr(0, 4);
        cv::putText(
          save, label_text, box.tl(), cv::FONT_HERSHEY_SIMPLEX, 0.6f, cv::Scalar(0, 255, 0), 2);
      }
    }
  }
...
void lite::utils::draw_boxes_with_landmarks_inplace(
  cv::Mat& mat_inplace, const std::vector<types::BoxfWithLandmarks>& boxes_kps, bool text)
{
...
cv::Mat lite::utils::draw_boxes_with_landmarks(
  const cv::Mat& mat, const std::vector<types::BoxfWithLandmarks>& boxes_kps, bool text)
{
...
cv::Mat lite::utils::draw_age(const cv::Mat& mat, types::Age& age)
{
  ...
  cv::Mat            canva    = mat.clone();
  const unsigned int offset   = static_cast<unsigned int>(0.1f * static_cast<float>(mat.rows));
  std::string        age_text = "Age:" + std::to_string(age.age).substr(0, 4);
  std::string        interval_text =
    "Interval" + std::to_string(age.age_interval[0]) + "_" + std::to_string(age.age_interval[1]);
  std::string interval_prob = "Prob:" + std::to_string(age.interval_prob).substr(0, 4);
  cv::putText(canva,
              age_text,
              cv::Point2i(10, offset),
              cv::FONT_HERSHEY_SIMPLEX,
              0.6f,
              cv::Scalar(0, 255, 0),
              2);
...
void lite::utils::draw_age_inplace(cv::Mat& mat_inplace, types::Age& age)
{
  ...
  const unsigned int offset =
    static_cast<unsigned int>(0.1f * static_cast<float>(mat_inplace.rows));
  std::string age_text = "Age:" + std::to_string(age.age).substr(0, 4);
  std::string interval_text =
    "Interval" + std::to_string(age.age_interval[0]) + "_" + std::to_string(age.age_interval[1]);
  std::string interval_prob = "Prob:" + std::to_string(age.interval_prob).substr(0, 4);
...
cv::Mat lite::utils::draw_gender(const cv::Mat& mat, types::Gender& gender)
{
  ...
  cv::Mat            canva       = mat.clone();
  const unsigned int offset      = static_cast<unsigned int>(0.1f * static_cast<float>(mat.rows));
  std::string        gender_text = std::to_string(gender.label) + ":" + gender.text + ":" +
                            std::to_string(gender.score).substr(0, 4);
  cv::putText(canva,
              gender_text,
              cv::Point2i(10, offset),
              cv::FONT_HERSHEY_SIMPLEX,
...
cv::Mat lite::utils::draw_emotion(const cv::Mat& mat, types::Emotions& emotions)
{
  ...
  cv::Mat            canva        = mat.clone();
  const unsigned int offset       = static_cast<unsigned int>(0.1f * static_cast<float>(mat.rows));
  std::string        emotion_text = std::to_string(emotions.label) + ":" + emotions.text + ":" +
                             std::to_string(emotions.score).substr(0, 4);
  cv::putText(canva,
              emotion_text,
              cv::Point2i(10, offset),
              cv::FONT_HERSHEY_SIMPLEX,
                           ...
                           float iou_threshold, unsigned int topk)
{
  ...
  std::sort(input.begin(), input.end(), [](const types::Boxf& a, const types::Boxf& b) {
                                  ...
                                  bool fgr_is_already_mul_pha)
{
  ...
  if (fgr_mat.type() != CV_32FC3)
    fgr_mat.convertTo(fg_mat_copy, CV_32FC3);   // 0.~255.
  else
    fg_mat_copy = fgr_mat;   // ref only
  ...
  if (!out_mat.empty() && out_mat.type() != CV_8UC3) out_mat.convertTo(out_mat, CV_8UC3);
}
