...
namespace tnncv {
class TNNSCRFD : public BasicTNNHandler
{
public:
  explicit TNNSCRFD(const std::string& _proto_path, const std::string& _model_path,
...
private:
  ...
  void generate_bboxes_single_stride(const SCRFDScaleParams&    scale_params,
                                     ...
                                     std::shared_ptr<tnn::Mat>& bbox_pred, unsigned int stride,
  ...
  void generate_bboxes_kps_single_stride(
    ...
    std::shared_ptr<tnn::Mat>& bbox_pred, std::shared_ptr<tnn::Mat>& kps_pred, unsigned int stride,
    float score_threshold, float img_height, float img_width,
    std::vector<types::BoxfWithLandmarks>& bbox_kps_collection);
