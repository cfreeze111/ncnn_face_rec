...
namespace tnncv {
class TNNUltraFace : public BasicTNNHandler
{
public:
  explicit TNNUltraFace(const std::string& _proto_path, const std::string& _model_path,
...
private:
  ...
  std::vector<float> bias_vals  = {
    -127.0f * (1.0f / 128.0f), -127.0f * (1.0f / 128.0f), -127.0f * (1.0f / 128.0f)};   // RGB
  enum NMS
  {
    HARD   = 0,
    BLEND  = 1,
