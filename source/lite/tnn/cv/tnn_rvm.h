...
namespace tnncv {
class TNNRobustVideoMatting
{
public:
  explicit TNNRobustVideoMatting(const std::string& _proto_path, const std::string& _model_path,
...
private:
  ...
  std::vector<std::string> input_names = {"src", "r1i", "r2i", "r3i", "r4i"};
  // hardcode output node names, hint only.
  std::vector<std::string> output_names           = {"fgr", "pha", "r1o", "r2o", "r3o", "r4o"};
...
private:
  ...
  void generate_matting(std::shared_ptr<tnn::Instance>& _instance, types::MattingContent& content,
                        int img_h, int img_w, bool remove_noise = false,
...
public:
   ...
   * See
   * https://github.com/PeterL1n/RobustVideoMatting/blob/master/documentation/inference_zh_Hans.md
  ...
  void detect_video(const std::string& video_path, const std::string& output_path,
                    std::vector<types::MattingContent>& contents, bool save_contents = false,
                    unsigned int writer_fps = 20, bool remove_noise = false,
                    bool minimum_post_process = false, const cv::Mat& background = cv::Mat());
};
}   // namespace tnncv
