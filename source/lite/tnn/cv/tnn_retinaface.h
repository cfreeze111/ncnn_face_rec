...
#ifndef LITE_AI_TOOLKIT_TNN_CV_TNN_RETINAFACE_H
#define LITE_AI_TOOLKIT_TNN_CV_TNN_RETINAFACE_H

#include "lite/tnn/core/tnn_core.h"

namespace tnncv {
class TNNRetinaFace : public BasicTNNHandler
{
public:
  explicit TNNRetinaFace(const std::string& _proto_path, const std::string& _model_path,
                         unsigned int _num_threads = 1);   //
  ~TNNRetinaFace() override = default;
...
private:
  ...
  std::vector<float> bias_vals   = {-104.f * 1.0f, -117.f * 1.0f, -123.f * 1.0f};   // bgr order
...
private:
  ...
  void generate_bboxes(std::vector<types::Boxf>&       bbox_collection,
                       std::shared_ptr<tnn::Instance>& _instance, float score_threshold,
                       float img_height,
...
public:
  void detect(const cv::Mat& mat, std::vector<types::Boxf>& detected_boxes,
              float score_threshold = 0.7f, float iou_threshold = 0.3f, unsigned int topk = 300,
              unsigned int nms_type = 0);
};
}   // namespace tnncv

#endif   // LITE_AI_TOOLKIT_TNN_CV_TNN_RETINAFACE_H
