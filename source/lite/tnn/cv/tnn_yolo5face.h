...
namespace tnncv {
class TNNYOLO5Face : public BasicTNNHandler
{
public:
  explicit TNNYOLO5Face(const std::string& _proto_path, const std::string& _model_path,
...
public:
  void detect(const cv::Mat& mat, std::vector<types::BoxfWithLandmarks>& detected_boxes_kps,
              float score_threshold = 0.3f, float iou_threshold = 0.45f, unsigned int topk = 400);
};
}   // namespace tnncv
