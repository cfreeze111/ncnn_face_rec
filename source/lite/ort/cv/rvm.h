...
private:
  ...
  void generate_matting(std::vector<Ort::Value>& output_tensors, types::MattingContent& content,
...
public:
   ...
   */
  void detect(const cv::Mat& mat, types::MattingContent& content, float downsample_ratio = 0.25f,
              bool video_mode = false, bool remove_noise = false,
  ...
  void detect_video(const std::string& video_path, const std::string& output_path,
                    ...
                    float downsample_ratio = 0.25f, unsigned int writer_fps = 20,
                    bool remove_noise = false, bool minimum_post_process = false,
