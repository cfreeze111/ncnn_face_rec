...
private:
  ...
  std::vector<const char*> output_node_names = {"alpha_os1", "alpha_os4", "alpha_os8"};
  ...
  unsigned int dynamic_input_image_size =
    1 * 3 * 512 * 512;   // init only, will change according to input mat.
  unsigned int dynamic_input_mask_size =
    1 * 1 * 512 * 512;   // init only, will change according to input mat.
  std::vector<int64_t> dynamic_input_image_dims = {
    1, 3, 512, 512};   // init only, will change according to input mat.
  std::vector<int64_t> dynamic_input_mask_dims = {
    1, 1, 512, 512};   // init only, will change according to input mat.
...
private:
  ...
  void update_alpha_pred(cv::Mat& alpha_pred, const cv::Mat& weight,
                         const cv::Mat& other_alpha_pred);
