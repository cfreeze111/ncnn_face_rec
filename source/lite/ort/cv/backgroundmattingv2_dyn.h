...
private:
  ...
  unsigned int dynamic_input_mat_size = 1;     // init only, will change according to input mat.
  unsigned int dynamic_input_bgr_size = 1;     // init only, will change according to input mat.
  ...
  std::vector<float> dynamic_input_mat_value_handler;
  std::vector<float> dynamic_input_bgr_value_handler;

protected:
  const unsigned int num_threads;   // initialize at runtime.

public:
  explicit BackgroundMattingV2Dyn(const std::string& _onnx_path, unsigned int _num_threads = 1);

  ~BackgroundMattingV2Dyn();

protected:
  BackgroundMattingV2Dyn(const BackgroundMattingV2Dyn&)            = delete;   //
  BackgroundMattingV2Dyn(BackgroundMattingV2Dyn&&)                 = delete;   //
  BackgroundMattingV2Dyn& operator=(const BackgroundMattingV2Dyn&) = delete;   //
  BackgroundMattingV2Dyn& operator=(BackgroundMattingV2Dyn&&)      = delete;   //
...
private:
  ...
  void generate_matting(std::vector<Ort::Value>& output_tensors, const cv::Mat& mat,
