...
public:
  explicit NCNNYoloR(const std::string& _param_path, const std::string& _bin_path,
...
private:
  ...
  void generate_bboxes_single_stride(const YoloRScaleParams& scale_params, ncnn::Mat& det_pred,
                                     unsigned int stride, float score_threshold, float img_height,
                                     float img_width, std::vector<types::Boxf>& bbox_collection);

  void generate_bboxes(const YoloRScaleParams& scale_params, ncnn::Extractor& extractor,
