...
#ifndef LITE_AI_TOOLKIT_NCNN_CV_NCNN_NANODET_DEPRECIATED_H
#define LITE_AI_TOOLKIT_NCNN_CV_NCNN_NANODET_DEPRECIATED_H

#include "lite/ncnn/core/ncnn_core.h"

namespace ncnncv {
class NCNNNanoDetDepreciated : public BasicNCNNHandler
{
public:
  explicit NCNNNanoDetDepreciated(const std::string& _param_path, const std::string& _bin_path,
                                  unsigned int _num_threads = 1, int _input_height = 320,
...
private:
  ...
  std::unordered_map<unsigned int, std::vector<NanoDepreciatedCenterPoint>> center_points;
...
private:
  ...
  void generate_bboxes_single_stride(const NanoDepreciatedScaleParams& scale_params,
                                     ncnn::Mat& cls_pred, ncnn::Mat& dis_pred, unsigned int stride,
                                     float score_threshold, float img_height, float img_width,
                                     std::vector<types::Boxf>& bbox_collection);

  void generate_bboxes(const NanoDepreciatedScaleParams& scale_params,
                       std::vector<types::Boxf>& bbox_collection, ncnn::Extractor& extractor,
...
public:
  ...
  void detect(const cv::Mat& mat, std::vector<types::Boxf>& detected_boxes,
              float score_threshold = 0.45f, float iou_threshold = 0.3f, unsigned int topk = 100,
              unsigned int nms_type = NMS::OFFSET);
};
}   // namespace ncnncv

#endif   // LITE_AI_TOOLKIT_NCNN_CV_NCNN_NANODET_DEPRECIATED_H
