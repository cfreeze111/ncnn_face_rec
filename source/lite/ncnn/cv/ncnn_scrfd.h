...
namespace ncnncv {
class NCNNSCRFD : public BasicNCNNHandler
{
public:
  explicit NCNNSCRFD(const std::string& _param_path, const std::string& _bin_path,
...
private:
  ...
  void generate_bboxes_single_stride(const SCRFDScaleParams& scale_params, ncnn::Mat& score_pred,
                                     ncnn::Mat& bbox_pred, unsigned int stride,
  ...
  void generate_bboxes_kps(const SCRFDScaleParams&                scale_params,
                           std::vector<types::BoxfWithLandmarks>& bbox_kps_collection,
                           ncnn::Extractor& extractor, float score_threshold, float img_height,
