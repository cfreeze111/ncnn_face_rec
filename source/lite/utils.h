...
namespace utils {
// Drawing Utils
::cv::Mat draw_axis(const ::cv::Mat& mat, const types::EulerAngles& euler_angles, float size = 50.f,
                    int thickness = 2);
...
::cv::Mat draw_boxes_with_landmarks(const ::cv::Mat&                             mat,
                                    const std::vector<types::BoxfWithLandmarks>& boxes_kps,
                                    bool                                         text = false);
...
void swap_background(const ::cv::Mat& fgr_mat, const ::cv::Mat& pha_mat, const ::cv::Mat& bgr_mat,
                     ::cv::Mat& out_mat, bool fgr_is_already_mul_pha = false);
...
template<typename T = float> std::vector<unsigned int> argsort(const std::vector<T>& arr);
template std::vector<unsigned int>                     argsort(const std::vector<float>& arr);

template<typename T = float> std::vector<unsigned int> argsort(const T* arr, unsigned int _size);
...
template<typename T>
std::vector<float> lite::utils::math::softmax(const T* logits, unsigned int _size,
                                              unsigned int& max_id)
{
...
template<typename T> std::vector<unsigned int> lite::utils::math::argsort(const std::vector<T>& arr)
{
  types::__assert_type<T>();
  if (arr.empty()) return {};
