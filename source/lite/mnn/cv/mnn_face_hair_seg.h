...
namespace mnncv {
class MNNFaceHairSeg : public BasicMNNHandler
{
public:
...
private:
  ...
  void generate_mask(const std::map<std::string, MNN::Tensor*>& output_tensors, const cv::Mat& mat,
                     types::FaceHairSegContent& content, bool remove_noise = false);

public:
  void detect(const cv::Mat& mat, types::FaceHairSegContent& content, bool remove_noise = false);
};
}   // namespace mnncv
