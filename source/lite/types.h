...
namespace lite {
namespace types {
template<typename _T1 = float, typename _T2 = float> static inline void __assert_type()
{
...
namespace types {
// bounding box.
template<typename T1 = float, typename T2 = float> struct BoundingBoxType
{
  ...
  // convert type.
  template<typename O1, typename O2 = score_type> BoundingBoxType<O1, O2> convert_type() const;
...
typedef struct SegmentationMaskContentType
{
  ...
  SegmentationMaskContentType()
    : flag(false){};
} SegmentationMaskContent;

typedef struct Photo2CartoonContentType
{
  ::cv::Mat cartoon;
  bool      flag;

  Photo2CartoonContentType()
    : flag(false){};
